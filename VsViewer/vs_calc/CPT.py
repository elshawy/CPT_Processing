...
class CPT:
    ...
    @staticmethod
    def from_file(cpt_ffp: str, cpt_info_path: str = "CPT_info.txt"):
    #def from_file(cpt_ffp: str, gwl: float = 1, nar: float = 0.8):
        """
        Creates a CPT from a CPT file, automatically applying gwl and nar
        from CPT_info.csv if matching CODE is found. If not found, defaults to gwl=1, nar=0.8.
        If the filename has insufficient underscores, the file is skipped (returns None).
        """
        cpt_ffp = Path(cpt_ffp)
        cpt_info_path = Path(cpt_info_path)

        # --- Extract CODE from filename (between two underscores)
        filename = cpt_ffp.stem
        parts = filename.split("_")
        if len(parts) < 3:
            print(f"Skipping file '{filename}': not enough underscores to extract CODE.")
            return None
        ...
        # --- Try loading CPT_info.csv and overriding if CODE found
        try:
            info_df = pd.read_csv(cpt_info_path)
            info_df["CODE"] = info_df["CODE"].astype(str).str.strip()

            row = info_df[info_df["CODE"].str.upper() == code.upper()]
            if not row.empty:
                # gwl
                if pd.notna(row.iloc[0].get("gwl", None)):
                    gwl = float(row.iloc[0]["gwl"])
                else:
                    gwl = 1.0

                # nar (a-factor)
                if pd.notna(row.iloc[0].get("a", None)):
                    nar = float(row.iloc[0]["a"])
                else:
                    nar = 0.8

                print(f"Applying gwl={gwl}, nar={nar} for CODE={code} (from {cpt_info_path})")
            else:
                print(f"CODE '{code}' not found in '{cpt_info_path}', using defaults gwl=1.0, nar=0.8")
        except FileNotFoundError:
            print(f"'{cpt_info_path}' not found, using defaults gwl=1.0, nar=0.8")

        # --- Read CPT CSV and process
        df = pd.read_csv(cpt_ffp)
